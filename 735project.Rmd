...
Here, for the test dataset, a lable of -1 means that this comment will not be used in the evaluation, so we delete those records from the dataset.
```{r}
...
Let's then take a look at the dimension and structure of the document-term matrices:
```{r}
dim(train.dtm)
...
ggplot(rsvm_results, aes(sigma, Kappa, col = factor(C))) + geom_point() + geom_line() + 
  ...
  ggtitle("Cross Validation Results of Radial SVM based on LASSO")
```
